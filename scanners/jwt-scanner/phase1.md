...
- **OIDC `id_token`의 서명/alg/iss/aud/exp 검증**은 본 스캐너 단독 담당. oauth-scanner는 OAuth 흐름 결함(nonce/audience mismatch)만, saml-scanner는 SAML assertion 서명 검증만.
- **SAML assertion 서명 결함** (XSW, comment injection 등)은 **saml-scanner** 단독 담당. 본 스캐너 후보 아님.
