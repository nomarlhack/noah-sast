...
각 후보는 `## <ID>: <제목>` 헤더로 시작하며, 아래 5개 필수 섹션을 포함한다. **`## ` (h2) 레벨은 후보 전용이다. 체크리스트 항목, "제외" 판정 항목 등 후보가 아닌 분석 내용은 `### ` (h3) 이하 레벨만 사용한다.**
...
- **작업 디렉토리에 여러 프로젝트가 존재하더라도, 분석 결과를 프로젝트별로 분리하지 않는다.** 모든 프로젝트의 결과를 하나의 흐름으로 통합하여 반환한다.
