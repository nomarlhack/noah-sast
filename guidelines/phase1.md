...
**여러 스캐너를 실행하는 경우, 스캐너별로 구분하여 반환한다. 구분자와 스캐너 태그는 정확히 아래 형식을 따른다:**
...
**[필수] 구분자와 스캐너 태그 규칙:**
- 스캐너 섹션 사이에 반드시 `===SCANNER_BOUNDARY===`를 삽입한다 (첫 스캐너 앞에도 포함).
- 각 섹션의 첫 줄은 `[스캐너명]` 형식이다 (예: `[xss-scanner]`). 스캐너명은 디렉토리명과 정확히 일치해야 한다.
- `=== xss-scanner ===` 같은 자유 형식 구분자를 사용하지 않는다.
