...
description: "취약점 스캔 보고서 MD 파일의 정확성을 실제 소스코드와 대조하여 검증하고, 부정확한 내용을 즉시 수정하는 스킬. 후보/확인됨 항목의 Source→Sink 흐름, 코드 스니펫, 부재 주장과 이상 없음 판정의 근거를 모두 검증한다. noah-sast가 HTML 변환 전에 호출하거나, 개별 스캐너 보고서에 대해서도 사용할 수 있다. 사용자가 '보고서 리뷰', '보고서 검증', 'review report', '내용 확인해줘', '보고서 정확한지 확인' 등을 요청할 때 이 스킬을 사용한다."
---
