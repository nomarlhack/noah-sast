---
name: scan-report
description: "취약점 스캔 결과를 보고서로 작성하는 스킬. noah-sast 또는 개별 스캐너가 분석을 완료한 후 이 스킬을 호출하여 보고서를 생성한다. 사용자가 '보고서 작성해줘', 'report 생성', '스캔 결과 보고서', '취약점 보고서' 등을 요청하거나, noah-sast/개별 스캐너가 분석 완료 후 보고서 작성 단계에서 이 스킬을 사용한다."
---
...
**SKILLS_DIR**: noah-sast에서 호출된 경우 이미 결정된 `<NOAH_SAST_DIR>` 값을 사용한다. 이 파일은 `<NOAH_SAST_DIR>/utils/scan-report/` 에 위치한다.
...
**[필수] 스켈레톤에 `## 스캐너별 실행 결과` 섹션과 `<!-- SCANNER_SECTIONS_HERE -->` 플레이스홀더를 반드시 포함한다. `## 미적용 스캐너 목록`은 플레이스홀더 뒤에 위치해야 한다.**
...
**조립 스크립트:** `<NOAH_SAST_DIR>/utils/scan-report/assemble_report.py`를 참조한다. 스크립트 내의 `skeleton`, `subagent_results`, `report_name` 변수를 설정한 후 Bash로 실행한다.
...
연계 분석을 수행하지 않았거나 체인이 없으면 이 섹션을 생략한다.
...
|------|--------|------|
| 1 | [후보 ID] | [이 단계에서 얻는 것] |
| 2 | [후보 ID] | [Step 1 결과를 이용한 다음 행동] |
| 3 | [후보 ID] | [최종 영향] |

#### 재현 방법 및 POC

**Step 1: [첫 번째 취약점 활용]**
체인의 첫 번째 취약점을 트리거하는 구체적인 curl 명령어와 설명.
...
**Step 2: [두 번째 취약점 활용]**
Step 1의 결과를 이용하여 다음 취약점을 트리거하는 curl 명령어.
...
- **[필수] 공격 시나리오에 재현 방법 및 POC 포함**: 공격 체인이 있는 경우, 각 체인에 `#### 재현 방법 및 POC` 섹션을 반드시 포함한다. 체인의 각 Step별로 구체적인 curl 명령어와 설명을 기재한다. 설명 문구(예: "연계 분석에서 도출된 공격 체인 N건을 기재한다")는 포함하지 않는다.
