...
2. **Source 식별**: 사용자가 제어 가능한 입력 진입점 (HTTP 파라미터, URL, Cookie, 클립보드, location 등). SPA에서는 API 응답 데이터(사용자가 작성한 댓글, 프로필, 게시글 등)도 Source에 해당한다.
...
사용자 입력 또는 관리자 입력이 source인 경우 후보. 서버 내부 설정값이 source인 경우 제외.
