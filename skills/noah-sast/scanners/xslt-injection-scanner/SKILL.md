---
name: xslt-injection-scanner
description: "소스코드 분석과 동적 테스트를 통해 XSLT Injection 취약점을 탐지하는 스킬. 사용자 입력이 XSLT 스타일시트나 XML 데이터에 반영되어 XSLT 프로세서에서 의도하지 않은 변환이나 코드 실행이 발생하는지 분석하고 검증한다. 사용자가 'XSLT injection 찾아줘', 'XSLT 스캔', 'XSLT 인젝션', 'XSL 변환 취약점', 'XSLT audit', 'XSLT 점검' 등을 요청할 때 이 스킬을 사용한다."
grep_patterns:
  - "TransformerFactory"
  - "XSLTProcessor"
  - "lxml\\.etree\\.XSLT"
