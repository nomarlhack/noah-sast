---
name: sourcemap-scanner
description: "웹 애플리케이션에서 Source Map 파일이 외부에 노출되어 있는지 확인하고, 노출된 Source Map에서 원본 소스코드, API 키, 시크릿, 내부 경로 등 민감한 정보가 포함되어 있는지 분석하는 스킬. 사용자가 'source map 찾아줘', 'sourcemap 스캔', '.map 파일 노출', 'source map 정보 노출', 'sourcemap audit', '소스맵 점검', '소스코드 노출' 등을 요청할 때 이 스킬을 사용한다."
grep_patterns:
  - "devtool"
  - "sourcemap"
  - "source-map"
