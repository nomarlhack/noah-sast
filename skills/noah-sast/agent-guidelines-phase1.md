...
- **프롬프트에 "패턴 인덱스 파일 경로"가 제공된 경우 (noah-sast에서 호출)**: Read 도구로 해당 파일(스캐너별 단독 JSON)을 읽어 패턴별 파일경로:라인번호 목록을 추출하고, 이를 필수 분석 대상으로 확정한다.
- **프롬프트에 "패턴 인덱스 파일 경로"가 없는 경우 (개별 실행)**: 해당 스캐너의 SKILL.md frontmatter에서 grep_patterns를 추출하여 소스코드 파일만 대상으로 직접 grep하여 파일 목록을 확정한다.
