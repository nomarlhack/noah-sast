...
```bash
python3 ~/.claude/skills/noah-sast/scanner-selector.py <PATTERN_INDEX_DIR> <PROJECT_ROOT>
```
...
- Phase 1 공통 지침: `~/.claude/skills/noah-sast/agent-guidelines-phase1.md`
...
먼저 아래 파일을 읽으세요:
- ~/.claude/skills/noah-sast/agent-guidelines-phase1.md
...
스캐너 1: xss-scanner
- phase1.md: ~/.claude/skills/xss-scanner/phase1.md
- 패턴 인덱스: <PATTERN_INDEX_DIR>/xss-scanner.json

스캐너 2: dom-xss-scanner
- phase1.md: ~/.claude/skills/dom-xss-scanner/phase1.md
- 패턴 인덱스: <PATTERN_INDEX_DIR>/dom-xss-scanner.json

스캐너 3: open-redirect-scanner
- phase1.md: ~/.claude/skills/open-redirect-scanner/phase1.md
...
Phase 1에서 후보가 2건 이상 발견된 경우, `~/.claude/skills/chain-analysis/SKILL.md`에 정의된 프로세스에 따라 **연계 분석 에이전트**를 실행한다. 후보가 1건 이하이면 이 단계를 건너뛴다.
...
사용자가 필요한 정보를 제공하면, 후보가 발견된 모든 스캐너에 대해 동적 분석을 수행한다. **[필수] 스캐너당 1 에이전트 원칙을 따른다. 여러 스캐너의 동적 분석을 하나의 에이전트에 묶지 않는다.** 각 에이전트 프롬프트에는 `~/.claude/skills/<scanner-name>/phase2.md`와 `~/.claude/skills/noah-sast/agent-guidelines-phase2.md` 경로를 포함한다.
...
Step 3에서 수집한 모든 개별 스캐너의 결과를 **`scan-report` 스킬(`~/.claude/skills/scan-report/SKILL.md`)에 전달**하여 통합 보고서를 생성한다.
...
**[필수] scan-report의 Step 3(MD 조립) 완료 후, Step 4(HTML 변환) 이전에 `scan-report-review` 스킬을 실행하여 보고서 정확성을 검증한다.** `~/.claude/skills/scan-report-review/SKILL.md`를 읽고, 보고서 MD 파일 경로와 프로젝트 루트 경로를 전달한다. 리뷰에서 부정확한 내용이 발견되면 MD 파일이 수정된 후 HTML 변환으로 진행한다.
...
- Phase 1 에이전트: `~/.claude/skills/noah-sast/agent-guidelines-phase1.md`
- Phase 2 에이전트: `~/.claude/skills/noah-sast/agent-guidelines-phase2.md`
