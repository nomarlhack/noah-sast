...
description: "소스코드 및 설정 분석과 동적 테스트를 통해 Subdomain Takeover 취약점을 탐지하는 스킬. DNS CNAME 레코드가 가리키는 외부 서비스(S3, GitHub Pages, Heroku 등)가 해제되어 공격자가 해당 서브도메인을 장악할 수 있는지 분석하고 검증한다. 사용자가 'subdomain takeover 찾아줘', '서브도메인 탈취', 'dangling DNS', 'CNAME takeover', '서브도메인 점검', 'subdomain audit' 등을 요청할 때 이 스킬을 사용한다."
grep_patterns:
  - "\\.github\\.io"
  - "\\.s3\\.amazonaws\\.com"
  - "\\.s3-website"
