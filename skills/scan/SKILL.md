---
name: scan
description: "41개 취약점 스캐너를 한번에 실행하고 결과를 통합 보고서로 작성하는 스킬. XSS, SSRF, SQLi, CSRF, TLS, 비즈니스 로직 등 모든 취약점 유형을 소스코드 분석과 동적 테스트로 점검한다. 사용자가 'noah-sast:scan', 'noah-sast', 'sast', '소스코드 취약점 스캔' 등을 요청할 때 이 스킬을 사용한다."
---
...
for dir in \
  ...
  "$HOME/.claude/skills/noah-sast" \
  ".claude/plugins/noah-sast" \
  "$HOME/.claude/plugins/noah-sast"; do
  if [ -d "$dir/scanners" ]; then
    cd "$dir" && pwd
    break
  fi
done
...
위 명령으로 경로를 찾지 못하면, 다음 fallback을 실행한다:

```bash
find "$HOME/.claude" -maxdepth 6 -name "plugin.json" -path "*noah-sast*" -print -quit 2>/dev/null | sed 's|/.claude-plugin/plugin.json$||'
```
...
<NOAH_SAST_DIR>/
  ...
  skills/scan/SKILL.md             ← 이 파일 (오케스트레이터)
  hooks/hooks.json                 ← 보안 후크 (파괴적 endpoint 차단)
  ...
  sub-skills/                      ← 내부 서브스킬 (플러그인에서 직접 호출)
    scan-report/
    scan-report-review/
    chain-analysis/
    webapp-testing/
