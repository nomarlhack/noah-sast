---
name: scan
description: "41개 취약점 스캐너를 한번에 실행하고 결과를 통합 보고서로 작성하는 스킬. XSS, SSRF, SQLi, CSRF, TLS, 비즈니스 로직 등 모든 취약점 유형을 소스코드 분석과 동적 테스트로 점검한다. 사용자가 'noah-8719:scan', 'sast', '소스코드 취약점 스캔' 등을 요청할 때 이 스킬을 사용한다."
---
...
```
${CLAUDE_PLUGIN_ROOT}/skills/scan
```
...
>   if [ -d "$base/skills/scan/scanners" ]; then cd "$base/skills/scan" && pwd; break; fi
> done
> ```
...
**디렉토리 구조:**
```
<NOAH_SAST_DIR>/                   ← ${CLAUDE_PLUGIN_ROOT}/skills/scan
