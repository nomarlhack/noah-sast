...
description: "소스코드 분석과 동적 테스트를 통해 WebSocket 취약점을 탐지하는 스킬. WebSocket 연결의 인증/인가 미흡, Cross-Site WebSocket Hijacking(CSWSH), Origin 검증 누락, 메시지 인젝션, 데이터 유출 등을 분석하고 검증한다. 사용자가 'WebSocket 취약점 찾아줘', 'WebSocket 스캔', 'ws:// 취약점', 'CSWSH', 'WebSocket hijacking', 'WebSocket 점검', 'WebSocket audit', '웹소켓 보안' 등을 요청할 때 이 스킬을 사용한다."
grep_patterns:
  - "new WebSocket("
  - "socket\\.io"
  - "WebSocket"
