...
description: "소스코드 분석과 동적 테스트를 통해 LDAP Injection 취약점을 탐지하는 스킬. 사용자 입력이 LDAP 쿼리(필터)에 반영되는 경로를 추적하고, 실제로 필터 로직을 변경하여 인증 우회나 정보 유출이 가능한지 검증한다. 사용자가 'LDAP injection 찾아줘', 'LDAP 인젝션 스캔', 'LDAP 필터 인젝션', 'LDAP 점검', 'LDAP audit', 'LDAP 인증 우회' 등을 요청할 때 이 스킬을 사용한다."
grep_patterns:
  - "ldapjs"
  - "activedirectory2"
  - "passport-ldapauth"
