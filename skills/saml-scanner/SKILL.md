---
name: saml-scanner
description: "소스코드 분석과 동적 테스트를 통해 SAML 인증 우회 취약점을 탐지하는 스킬. SAML Response/Assertion의 서명 검증, XML 처리, 속성 조작 등을 분석하고, 실제로 서명 제거/래핑 공격으로 인증을 우회할 수 있는지 검증한다. 사용자가 'SAML 취약점 찾아줘', 'SAML 스캔', 'SAML 인증 우회', 'signature wrapping', 'SAML assertion 변조', 'SAML audit', 'SSO 취약점' 등을 요청할 때 이 스킬을 사용한다."
grep_patterns:
  - "saml"
  - "SAML"
  - "omniauth-saml"
