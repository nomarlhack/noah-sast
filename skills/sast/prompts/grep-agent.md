...
41개 취약점 스캐너의 grep 패턴을 각 스캐너 phase1.md frontmatter에서 읽어,
...
- Python/Shell/Node 등으로 grep 실행을 수행할 **스크립트 파일을 작성만 하고 실행하지 않은 채 종료**
- 스크립트 파일(`run_scan.py`, `runner.py`, `run_grep.sh`, `_batch*.json` 등)을 `<PATTERN_INDEX_DIR>`에 남기고 JSON 인덱스만 **미완성** 상태로 반환
- "스크립트 작성으로 실행 계획을 완성했다"는 판단으로 실제 grep 호출 없이 종료

**[필수]** grep 실행 방식:

- Bash 도구로 `grep` 명령을 **직접 호출**한다. 한 호출에 1개 패턴을 쿼리한다(또는 여러 패턴을 `-e`로 묶어 한 번에 호출해도 무방).
- 스크립트로 일괄 실행이 효율적으로 보여도 **이 에이전트 세션 안에서 Bash 도구로 실제 실행되어 JSON이 생성되지 않으면 결과물이 아니다**.
- 컨텍스트/시간 예산 부족 시 **진행한 만큼만 JSON에 저장**하고 미처리 스캐너를 `[INCOMPLETE: scanner-name]`으로 반환한다. 메인 에이전트가 미완 스캐너만 재호출한다.

**[필수]** 반환 전 자기 검증:

1. Bash `ls -1 <PATTERN_INDEX_DIR>/*.json 2>/dev/null | wc -l` 실행
2. 출력값이 **처리한 스캐너 수와 일치**하는지 확인
3. 0 또는 불일치면 누락 스캐너를 재실행한 뒤 다시 검증. 여전히 불일치면 `[INCOMPLETE: scanner-name]` 표기로 반환

## 단계 1: 스캐너별 패턴 수집 (phase1.md frontmatter)

아래 41개 스캐너의 phase1.md를 각각 Read 도구로 읽는다:
...
<NOAH_SAST_DIR>/scanners/cookie-security-scanner/phase1.md
<NOAH_SAST_DIR>/scanners/tls-scanner/phase1.md
<NOAH_SAST_DIR>/scanners/validation-logic-scanner/phase1.md
```
...
- 히트 없는 패턴도 빈 배열로 포함.
- 41개 스캐너 전체 각각 저장. 누락 금지. (business-logic-scanner는 grep_patterns가 비어 있으므로 빈 JSON `{}` 저장)

## 에러 처리 및 자기 검증

**에러 처리:**
- phase1.md Read 실패 시: 해당 스캐너를 건너뛰고, 반환에 `[SKIP: phase1.md 없음 — scanner-name]`을 표기한다.
- Write 실패 시: 반환 메시지에 해당 스캐너의 카운트 요약을 포함하고 `[FALLBACK: Write 실패]`를 표기한다.
- 컨텍스트 예산: 41개 스캐너 처리 중 완료가 불확실하면, 완료분을 먼저 저장하고 미완료 스캐너를 `[INCOMPLETE: scanner-name]`으로 표기한다.

**자기 검증 (반환 전 필수):** 저장된 JSON 파일 수가 처리한 스캐너 수와 일치하는지 확인한다. 불일치 시 누락 스캐너를 반환에 표기한다. 상세 절차는 상단 "실행 규약" 참조.

**임시 산출물 금지:** `run_*.py`, `_runner*.py`, `_run_*.sh`, `_batch*.json`, `_summary.json`, `_scanners.json` 등 grep 호출을 미루기 위한 중간 산출물 파일을 `<PATTERN_INDEX_DIR>`에 **남겨서는 안 된다**. 해당 디렉토리에는 오직 `<스캐너명>.json` 41개만 존재해야 한다.
