...
**[필수] status 필드를 manifest에 기록하지 않는다.** status 할당은 `scan-report-review`가 `mode=evaluate`로 호출될 때 수행한다 (`sub-skills/scan-report-review/checklist.md` §10 판정 플로우).
...
**해당 없는 필드는 생략**한다. `null` placeholder 금지 (checklist.md §10의 "기록을 위한 기록" 방지 규약).
...
- status 할당은 `scan-report-review`가 `mode=evaluate`로 수행한다 (`sub-skills/scan-report-review/checklist.md` §10~§11).
...
Phase 2 에이전트가 수집해야 할 정보는 판정자가 checklist.md §10 판정 플로우를 수행할 수 있는 수준이어야 한다.
