...
mode=evaluate 완료 이후에 연계 분석이 실행되는지 런타임 assert한다.

Phase 2 우선 원칙에 따라 Phase 1↔Phase 2 불일치는 evaluate가 Phase 2
...
Exit code (sub-skills/scan-report-review/_contracts.md §2 Exit Code 통일 테이블):
  ...
  4: reopen_pending — evaluate_phase1 재호출 권장 (품질 개선 힌트, 파이프라인 차단 아님)
...
def main() -> int:
    ...
    if safe_no_cat:
        ...
        print(
            "복구: evaluate_phase1을 FORCE_REOPEN="
            + ",".join(safe_no_cat[:10])
    ...
    #   - defense_verified (evaluate writer) → verified_defense 객체 필수
    #   - no_external_path / false_positive / not_applicable (evaluate_phase1 writer)
    ...
    if manifest_status_violations:
        ...
        for fname, rid in manifest_status_violations[:5]:
            print(f"  - {fname} :: {rid}")
        print("_contracts.md §4 제약 위반. Phase 2 에이전트가 status를 할당하면 안 됨 (evaluate 전용).")
        return 1
