#!/usr/bin/env python3
"""
blind_read_phase1_md.py — evaluate_phase1 전용 헬퍼.

Phase 1 결과 MD를 읽어 Phase 1 평가자의 구조적 판정 단서를 마스킹한 뷰를 반환한다.
이는 blind eval의 목적인 "확증 편향 완화"를 위한 전처리이다.

Usage:
    python3 blind_read_phase1_md.py <phase1_md_path>   # 마스킹 뷰를 stdout에 출력

Mask 대상 (sub-skills/scan-report-review/evaluate_phase1.md §blind eval 메커니즘):
    - "### Decision"
    - "### Confidence"
    - "### 판정 요약"
    - "### Status"
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: blind_read_phase1_md.py <phase1_md_path>", file=sys.stderr)
        sys.exit(1)
