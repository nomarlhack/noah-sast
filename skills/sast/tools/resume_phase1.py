...
Usage:
  python3 resume_phase1.py <PHASE1_RESULTS_DIR>

판별 대상:
  - _expected_scanners.json (scanner-selector.py 산출)
...
def main() -> int:
    if len(sys.argv) < 2:
        print("Usage: resume_phase1.py <PHASE1_RESULTS_DIR>", file=sys.stderr)
        return 1
    ...
    if not expected_file.is_file():
        ...
        print(f"ACTION: scanner-selector.py 미실행. Step 2-1부터 재시작.")
        return 0
    ...
    if not completed and not invalid and not missing:
        print(f"  {step}. scanner-selector.py 재실행하여 그룹 편성 복구 후 전체 dispatch")
        step += 1
    elif redispatch:
        print(
            f"  {step}. phase1-group-agent 재dispatch: 남은 스캐너 {redispatch}. "
            "원 그룹 편성이 컨텍스트에 없으면 scanner-selector.py 재실행하여 편성 복구"
        )
        step += 1
    if ai_action:
    ...
    if state != "initial" and (redispatch or ai_action or master_stale):
        print(f"  {step}. build-master-list.py 재실행")
        step += 1
    if state == "complete":
