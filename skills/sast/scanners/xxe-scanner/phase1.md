...
- **SAML response 파싱 XXE** (`SAML_XXE` 라벨)는 본 스캐너 단독 담당. saml-scanner는 xxe-scanner로 위임만 한다.
- **XSLT `document()`/`unparsed-text()` 함수**에 의한 외부 리소스 접근(SSRF 효과)은 **xslt-injection-scanner** 단독 담당. 본 스캐너는 **XML parser의 entity expansion**만.
- **SVG/OOXML 업로드 내 XML 파서 결함** (`OOXML_XXE` 라벨)은 본 스캐너 단독.
