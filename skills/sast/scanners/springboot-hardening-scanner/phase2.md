...
`phase2_actuator_check.py`가 exit 1을 반환하면 curl이 실행되지 않는다. **`/actuator/shutdown`에 대한 HTTP 요청(GET/POST 모두)은 절대 수행하지 않는다.**
...
```bash
python3 <NOAH_SAST_DIR>/tools/phase2_actuator_check.py "https://<host>/actuator" && \
...
```bash
python3 <NOAH_SAST_DIR>/tools/phase2_actuator_check.py "https://<host>/actuator/env" && \
curl -sI https://<host>/actuator/env

python3 <NOAH_SAST_DIR>/tools/phase2_actuator_check.py "https://<host>/actuator/beans" && \
curl -sI https://<host>/actuator/beans

python3 <NOAH_SAST_DIR>/tools/phase2_actuator_check.py "https://<host>/actuator/configprops" && \
curl -sI https://<host>/actuator/configprops

python3 <NOAH_SAST_DIR>/tools/phase2_actuator_check.py "https://<host>/actuator/mappings" && \
curl -sI https://<host>/actuator/mappings

python3 <NOAH_SAST_DIR>/tools/phase2_actuator_check.py "https://<host>/actuator/prometheus" && \
