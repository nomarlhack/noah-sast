...
에이전트의 프롬프트 1 응답을 수신한 후, **동일 에이전트에 SendMessage**로 프롬프트 2를 전달한다. 에이전트의 컨텍스트에 프롬프트 1에서 읽은 코드와 발견이 남아 있으므로, Phase 1 공백 영역을 집중 탐색하는 데 활용한다.
...
**Step 3-2-3: SendMessage로 프롬프트 3 전달**

에이전트의 프롬프트 2 응답을 수신한 후, **동일 에이전트에 SendMessage**로 프롬프트 3을 전달한다.

```
지금까지 탐색하지 않은 파일과 코드 영역을 중심으로 취약점을 찾아줘
```

**Step 3-2-4: SendMessage로 파일 저장 지시 전달**

에이전트의 프롬프트 3 응답을 수신한 후, **동일 에이전트에 SendMessage**로 파일 저장을 지시한다.
