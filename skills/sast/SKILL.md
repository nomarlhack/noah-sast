...
**동적 테스트 결과로 기존 보고서 섹션을 갱신해야 하는 경우 (재호출 경로)**:

초기 Step 3-3에서 사용자가 "소스코드 분석만"을 선택했다가 이후 동적 테스트를 추가로 요청한 경우, 또는 새로 확인됨 증거가 확보된 경우, 메인 에이전트는 **절대로 기존 sr_*.md 파일을 수동 sed/편집으로 고치지 않는다**. 대신:

1. `SANDBOX_DOMAINS`를 재확정(사용자 확인 포함)
2. 해당 후보가 속한 스캐너의 **scan-report Step 2 서브에이전트를 재호출**한다. 프롬프트에 확정된 `SANDBOX_DOMAINS`와 확인됨 증거(HTTP 응답 일부, 관찰된 페이로드 등)를 전달
3. 서브에이전트가 확인됨 상태 + 실제 관찰 결과 + sandbox 도메인을 포함한 섹션을 새로 작성하여 반환
4. `assemble_report.py`로 보고서를 재조립

이 방식은 메인 에이전트가 vuln-format.md의 필드 구조를 우회하거나 URL 플레이스홀더를 부정확하게 치환하는 위험을 원천 차단한다.
