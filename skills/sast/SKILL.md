...
**[필수] 메인 에이전트는 AI 자율 탐색 에이전트 프롬프트 본문을 인라인으로 복사하지 않는다.** 대신 아래 형식의 짧은 프롬프트를 사용하되, `<NOAH_SAST_DIR>`/`<PATTERN_INDEX_DIR>`/`<PHASE1_RESULTS_DIR>` 변수를 resolve된 실제 경로 문자열로 치환한다.

```
<NOAH_SAST_DIR>/prompts/ai-discovery-agent.md를 Read 도구로 읽고 그 안의 지시를 정확히 따르세요.
변수: NOAH_SAST_DIR=<NOAH_SAST_DIR>, PATTERN_INDEX_DIR=<PATTERN_INDEX_DIR>, PHASE1_RESULTS_DIR=<PHASE1_RESULTS_DIR>
...
에이전트는 모든 프롬프트 완료 후 결과를 **`<PHASE1_RESULTS_DIR>/ai-discovery.md`에 Phase 1 호환 형식으로 저장**한다 (후보별 5개 필수 섹션 + manifest 블록). 이 파일은 하류의 동적 분석(Step 3-5), 연계 분석(Step 3-6), 보고서(Step 4)에서 Phase 1 스캐너 결과 파일과 동일하게 참조된다.

에이전트가 반환한 후보 건수 요약을 확인한 후:
