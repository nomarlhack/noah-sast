...
>
> ```bash
> for base in "$HOME/.claude/skills/noah-8719" "$HOME/.claude/plugins/noah-8719"; do
>   if [ -d "$base/skills/sast/scanners" ]; then cd "$base/skills/scan" && pwd; break; fi
> done
> ```
...
프롬프트: 이 프로젝트의 소스코드를 분석하여 보안 취약점을 찾아줘
```

`<MASTER_LIST_SUMMARY>`에는 `<PHASE1_RESULTS_DIR>/master-list.json`에서 각 후보의 ID, 유형, 위치(file:line), Source→Sink 경로를 간략히 나열한다.

**Step 3-2-2: SendMessage로 프롬프트 2 전달**

에이전트의 프롬프트 1 응답을 수신한 후, **동일 에이전트에 SendMessage**로 프롬프트 2를 전달한다. 에이전트의 컨텍스트에 프롬프트 1에서 읽은 코드와 발견이 남아 있으므로, 이를 활용한 확장 탐색이 가능하다.

```
정적 분석 결과를 참고하여, 발견된 취약점과 관련된 다른 취약점을 찾아줘
```

**Step 3-2-3: SendMessage로 프롬프트 3 + 파일 저장 지시 전달**

에이전트의 프롬프트 2 응답을 수신한 후, **동일 에이전트에 SendMessage**로 프롬프트 3과 파일 저장 지시를 전달한다.

```
살펴보지 않은 다른 포인트에서 취약점을 찾아줘.
이것이 마지막 프롬프트이다. 탐색 완료 후, 지금까지 발견한 모든 후보를 통합하여 ai-discovery-agent.md의 "결과 파일 형식"에 따라 <PHASE1_RESULTS_DIR>/ai-discovery.md에 저장하라.
```
