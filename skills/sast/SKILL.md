...
정적 분석(Step 3-1)은 선별된 모든 스캐너 에이전트를 **단일 응답에서 동시에** Agent 도구로 실행한다. AI 자율 탐색(Step 3-2)은 구조화된 스캐너가 놓칠 수 있는 취약점을 AI가 코드를 직접 읽으며 발견한다. 동적 분석(Step 3-5)은 세션 충돌 방지를 위해 순차 실행한다. 연계 분석(Step 3-6)은 동적 분석 결과(확인됨/후보/안전)를 반영하여 공격 시나리오를 도출한다. 각 스캐너는 자신의 phase1.md(소스코드 분석) → phase2.md(동적 테스트) 프로세스를 따른다.
...
Phase 1 정적 분석 완료 후, 구조화된 스캐너가 놓칠 수 있는 취약점을 AI가 자율적으로 탐색한다. 메인 에이전트가 **SendMessage로 프롬프트를 1개씩 순차 전달**하여, 이전 프롬프트에서 읽은 코드와 발견한 인사이트가 에이전트 컨텍스트에 실제로 누적되도록 한다.
...
> **총 5개 메시지** (브리핑 1 + 탐색 3 + 저장 1). 위 표는 탐색 메시지 3개의 의미적 뷰이며, 실제 오케스트레이션에는 브리핑과 저장 지시가 별도 메시지로 추가된다.

**실행 절차:**

**Step 3-2-1: 에이전트 생성 (브리핑 전달)**

Agent 도구로 AI 자율 탐색 에이전트를 생성한다. `<NOAH_SAST_DIR>`/`<PHASE1_RESULTS_DIR>` 변수를 resolve된 실제 경로 문자열로 치환한다. **이 메시지는 브리핑 전용**이며 탐색 지시를 포함하지 않는다 — 에이전트가 먼저 지침서를 Read하고 내부 상태를 갖춘 후 다음 메시지부터 탐색을 시작한다.

```
<NOAH_SAST_DIR>/prompts/ai-discovery-agent.md를 Read 도구로 읽고 그 안의 지시를 정확히 따르세요. 이 메시지는 브리핑이며, 이어지는 SendMessage로 탐색 프롬프트를 1개씩 순차 전달한 뒤 마지막에 저장 지시를 보낼 것이다.
...
**Step 3-2-2: SendMessage로 탐색 프롬프트 1 전달**

브리핑 응답을 수신한 후, **동일 에이전트에 SendMessage**로 첫 탐색 프롬프트를 전달한다. 스캐너 카테고리에 구속되지 않는 자유 탐색이다.

```
이 프로젝트의 소스코드를 분석하여 보안 취약점을 찾아줘
```

**Step 3-2-3: SendMessage로 탐색 프롬프트 2 전달**

탐색 프롬프트 1 응답을 수신한 후, **동일 에이전트에 SendMessage**로 프롬프트 2를 전달한다. 에이전트의 컨텍스트에 프롬프트 1에서 읽은 코드와 발견이 남아 있으므로, Phase 1 공백 영역을 집중 탐색하는 데 활용한다.

```
master-list.json에서 파악한 Phase 1 커버리지(취약점 유형, 파일, 라인)를 바탕으로, Phase 1이 다루지 않은 영역을 집중 탐색해줘. 특히 비즈니스 로직 결함, 인증·인가 흐름 전체 경로, Race Condition, Mass Assignment처럼 grep 패턴으로는 잡기 어려운 취약점에 집중해줘
```

**Step 3-2-4: SendMessage로 탐색 프롬프트 3 전달**

탐색 프롬프트 2 응답을 수신한 후, **동일 에이전트에 SendMessage**로 프롬프트 3을 전달한다.

```
지금까지 탐색하지 않은 파일과 코드 영역을 중심으로 취약점을 찾아줘
```

**Step 3-2-5: SendMessage로 파일 저장 지시 전달**

탐색 프롬프트 3 응답을 수신한 후, **동일 에이전트에 SendMessage**로 파일 저장을 지시한다. 이 메시지는 독립된 턴이며 추가 탐색 지시를 포함하지 않는다.

```
이것이 마지막 프롬프트이며, 저장 지시이다. 먼저 ai-discovery-agent.md의 "후보 등록 제외 기준" 7개 항목을 지금까지 발견한 모든 후보에 적용하여 걸러낸다 (Source 통제 불가 / 트리거 경로 미확정 / 부재 주장 미입증 / 외부 전제 필요 / 서버 검증 확인됨 / dev·test 전용 / 전이 의존성 한정). 통과한 후보만 통합하여 "결과 파일 형식"에 따라 <PHASE1_RESULTS_DIR>/ai-discovery.md에 저장하라.
...
- 기존 카테고리에 속하지 않는 AI 후보(예: Race Condition, Mass Assignment 등)는 메인 에이전트가 `guidelines-phase2.md` 지침을 따라 직접 동적 테스트를 수행한다.
