...
**[필수] 백그라운드 작업 대기 시 `sleep` 사용 금지.**

- 오래 걸리는 Bash 호출은 `run_in_background: true` 옵션으로 실행한다. Claude Code가 완료 시 자동 알림을 보낸다.
- 알림 수신 전까지 메인 에이전트는 polling/`sleep` 체인을 사용하지 않는다. 60초 이상 leading `sleep`은 Claude Code가 차단한다.
- 외부 상태(파일 생성, 로그 갱신 등) 관찰이 필요한 경우에만 `Monitor` 도구와 짧은 `until` 루프를 사용한다.
