...
상세: `sub-skills/scan-report-review/evaluate.md` §Idempotent 동작.
...
| `4` | reopen_pending (품질 개선 힌트) | No | `evaluate_phase1` 선택적 재호출. Phase 2가 status를 확정했으므로 파이프라인 진행 가능 |
...
| 모드 | AI 후보 처리 |
|------|-------------|
| `evaluate_phase1` | `ai-discovery.md`를 원본으로 보고 `evaluation/ai-discovery-eval.md`에 판정 기록. 4개 검증 축·blind eval 절차 동일 적용 |
...
| `review` | 보고서의 `## AI 자율 탐색 결과` 섹션도 9개 축으로 동일하게 검증 |
...
- 모드별 절차: `skills/sast/sub-skills/scan-report-review/evaluate_phase1.md` / `evaluate.md` / `review.md`
