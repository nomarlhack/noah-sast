...
## 항목 B: `select_scanners.py`에 Tier 컬럼 추가
...
`select_scanners.py` 출력에 Tier 필드를 추가:
...
Step 3-5의 Tier 표 삭제 → `select_scanners.py`가 출력하는 Tier 정보 사용하도록 1줄 지침:

> Tier 분류는 `select_scanners.py` 출력의 `--- Tier 요약 ---` 섹션을 따른다. 메인 에이전트는 Tier 간 병렬, Tier 내 순차로 Phase 2 에이전트를 디스패치한다.
...
- 단점: 기존 `select_scanners.py`에 상수 딕셔너리 추가. 테스트 fixture 갱신 필요.

---

## 항목 A: `report_finalize.sh` 쉘스크립트
...
`tools/report_finalize.sh`:

```bash
#!/bin/bash
# Usage: report_finalize.sh <NOAH_SAST_DIR> <PHASE1_RESULTS_DIR> <confirmed_candidate_count>
...
2. 나머지 검증·변환·열기: `report_finalize.sh <NOAH_SAST_DIR> <PHASE1_RESULTS_DIR> <confirmed+candidate 건수>`
```
...
```bash
python3 <NOAH_SAST_DIR>/tools/phase1_ai_renumber.py <PHASE1_RESULTS_DIR>/ai-discovery.md
```
...
|------|----------|---------------|
| B | select_scanners.py 확장 | ~20줄 |
| A | report_finalize.sh 신규 | ~20줄 |
| D | grep_index.sh 신규 | ~10줄 |
| C | phase1_ai_renumber.py 신규 | ~3줄 |
| **합계** | **신규 3 + 확장 1** | **~53줄** |
...
`phase2_review_assert.py`에 체크리스트 출력 플래그 추가 (`--print-checklist`)만으로도 상당 부분 해결 가능. 다만 이건 체크리스트의 "동적 테스트 실행 결과 파트 검증" 부분을 완전히 자동화하지는 못함.
...
- **쉘스크립트 포팅 이슈**: `report_finalize.sh`, `grep_index.sh` 모두 macOS/Linux 양쪽 동작 필요. `ls`, `wc`, `tr` 같은 POSIX 명령만 사용해 호환성 확보.
