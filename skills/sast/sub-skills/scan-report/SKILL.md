...
> - 각 상세 섹션 `#### N. 제목` 헤딩 **바로 다음 줄**에 `**ID**: <master-list.candidates[].id>`를 포함한다. ID는 프롬프트로 전달된 master-list id를 그대로 사용한다. 한 섹션에 `**ID**:` 라인은 정확히 1회만 존재한다. 이 필드는 `assemble_report.py`가 요약 테이블 "ID" 칼럼을 채우고 `validate_report.py`가 연계 시나리오/안전 판정 섹션의 ID 참조를 교차검증하는 단일 진실 원천이다.
