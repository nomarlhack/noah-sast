...
보고서의 `## 안전 판정 항목` 섹션은 **safe로 도달한 이유 유형별로 4개 소분류**한다. 동일 결과라도 근거가 다르면 회귀 리스크·수정 우선순위가 다르므로 독자가 구분할 수 있어야 한다.

내부 용어(§, mode명, "구조적 폐기", "Source 도달성" 등)는 제목에 쓰지 않는다. 평이한 한국어 제목 + 한 줄 설명 + 테이블로 구성한다.
