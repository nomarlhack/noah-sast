...
| 플랫폼 기본 방어 확인 | 브라우저/HTTP/런타임 기본 동작이 동등 효과의 방어 제공 | phase1-review | 헤더 명시 부재지만 브라우저 기본값이 동일 효과 |
| 위협 모델 저영향 | Source 제어자가 신뢰 당사자 또는 엔드포인트 최대 영향이 낮음 | phase1-review | 관리자 전제, 저영향 엔드포인트, 셀프 영향 한정 |
...
### 위협 모델 저영향 (N건)
### 아키텍처 근거 중복 (N건)
(위 3종은 각 소분류 표의 4번째 열을 각각 "동등 방어 메커니즘" / "영향 한정 근거" / "경로 증거 대상 후보"로 두고 동일 형식)
```
