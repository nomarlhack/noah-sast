...
> 위 5개 필드는 개요의 **필수 필드**이며 모든 보고서에 동일 순서로 포함한다.
> 보고 대상에 따라 추가 필드를 더할 수 있다(예: 정책 범위, 감사 ID 등).
> 단 내부 파이프라인 서술(`Phase N`, `evaluate_phase1`, `내부 파이프라인`,
> `mode=...` 등)은 독자 레이어 금지 용어이므로 개요에 포함하지 않는다.
> 이는 `scan-report/vuln-format.md`의 "독자 레이어 노출 금지 용어" 규약과
> `tools/lint_reader_layer.py`의 lint 대상과 일관되게 적용된다.
>
