...
4. **정확성 Spot-check** (✓ 유지 항목 대상):
   - 각 서브에이전트의 반환에서 `✓ 유지` 항목을 추출하고, 서브에이전트당 최대 2개를 샘플링한다 (1개만 있으면 1개, 0개면 스킵).
   - 각 샘플에서 `(source: <표현식>, <파일>:<라인>)` 부분을 추출한다.
   - 해당 `<파일>:<라인>`을 Read 도구로 읽어 `<표현식>`(또는 의미적으로 동등한 코드)이 실제로 존재하는지 확인한다.
   - 표현식이 해당 위치에 없거나 의미가 다르면 **허위 증거**로 간주하고, 해당 서브에이전트가 반환한 **모든** ✓ 유지 항목을 재검증 대상으로 지정하여 새 서브에이전트를 디스패치한다.
...
### Step 5.5: 승격 후보 Source 도달성 보완 검증

Step 5에서 이상 없음 → 후보로 승격된 항목이 있으면, 해당 항목에 대해 Source 도달성 검증을 수행한다.

1. Step 5에서 신규 작성된 후보 상세 섹션 목록을 추출한다.
2. 각 승격 후보에 대해 checklist.md §9 + §4-1(e) Source 도달성 검증을 수행하는 서브에이전트를 디스패치한다.
3. 검증 결과가 `✗ 폐기`이면 승격을 취소하고 이상 없음으로 원복한다.
4. 승격 후보의 Source 도달성 판정을 Step 4.5의 집계에 반영한다.
...
**게이트 2 — 보고서 일관성**: 수정된 MD 파일에서 `#### N. 제목` 헤딩을 재카운트하여 M_candidates_final을 독립 산출한다. 수정 내역의 변동과 대조하여 일치를 검증한다:
M_candidates_final == M_candidates − 전체_제거_수 + 승격_생존_수 − merge_absorbed_count
(전체_제거_수 = 원래 후보(M_candidates 대상)에서 발생한 후보→안전 재분류 건수. Source 폐기 + 부재 주장 거짓 + 기타 사유를 모두 포함한다. Step 5.5 승격 취소(원복)는 원래 후보가 아니므로 포함하지 않는다. 공식이 아닌 MD 재카운트로 좌변을 산출하여 Step 5 수정 누락도 탐지)
