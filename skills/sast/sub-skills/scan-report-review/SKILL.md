...
**게이트 2 — 보고서 일관성**: 수정된 MD 파일에서 `#### N. 제목` 헤딩을 재카운트하여 M_candidates_final을 독립 산출한다. 유지_count + 불명확_count == M_candidates_final + merge_absorbed_count이어야 한다.
(보고서의 실제 후보 수가 비폐기 판정 수와 일치하는가. 공식이 아닌 MD 재카운트로 산출하여 Step 5 수정 누락도 탐지. merge_absorbed_count = checklist item 8 통합에 흡수된 항목 수, 통합 없으면 0)
