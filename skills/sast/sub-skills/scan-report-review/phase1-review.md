...
Source 도달성이 ✓ 유지라도 다음 기준에 해당하면 DISCARD 우선 검토한다. 해당 카테고리는 `_contracts.md §3 safe_category enum` 참조.

- **플랫폼 기본 방어**: 브라우저/HTTP/런타임 기본 동작이 동등 효과 방어를 제공 → `platform_default_defense`
- **위협 모델 저영향**: Source 제어자가 신뢰 당사자이거나 엔드포인트 최대 영향이 낮음 → `low_threat_model` (피해가 타 사용자로 확산되면 부적합 — `_contracts.md §3` 경계 참조)
- **아키텍처 근거 중복**: 다른 후보의 경로 증명용으로만 기술된 독립 항목 → `architectural_rationale_only`

**복수 성립 시 우선순위**: `no_external_path` / `not_applicable` > `defense_verified` / `platform_default_defense` > `low_threat_model` > `architectural_rationale_only`.

의심이 있으면 보수적 CONFIRM 유지 (폐기 기준은 명시적 근거가 있을 때).
