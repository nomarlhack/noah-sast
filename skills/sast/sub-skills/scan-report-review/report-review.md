...
문제: [체크리스트 번호 + 구체 내용]
실제: [소스/데이터에서 확인한 실제]
권장 조치: [보고서 해당 항목 수동 재검토 / 사용자 판단 필요 등 — 자동 조치 경로 없음]
```
...
report-review는 master-list·eval·보고서를 수정할 수 없으므로 아래 항목을 사용자에게 보고한다. 사용자가 보고서를 읽을 때 참고.

| ID | 문제 | 비고 |
|----|------|------|
| XSS-2 | Source 도달성 ✗ 폐기: code prop이 static 리터럴 (체크리스트 9) | Phase 1 판정 의심 — 보고서 해당 항목 수동 재검토 필요 |
| SSRF-3 | 이상 없음이 실은 취약: URL 파싱 경로 누락 (체크리스트 6) | phase2-review가 놓친 후보 — 보고서 이상 없음 스캐너 테이블 수동 재검토 필요 |
```
