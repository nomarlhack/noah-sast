...
| `phase1_eval_state.reopen` (품질 개선 힌트, 비차단) | `phase2-review` (set + Phase 2 재평가 완료 후 자체 reset) 및 `phase1-review` (처리 후 reset + `FORCE_REOPEN` 수신 시 self-set). `report-review`는 반환 텍스트의 `## 재평가 요청` 섹션으로 요청만 | `report-review`(요청만), 연계 분석 |
...
- `safe_category` 부재 + `status == safe`: **exit 7 차단**. 복구: `phase1-review`을 `FORCE_REOPEN=<IDs>`로 재호출하여 재기록.
