---
name: csv-injection-scanner
description: "소스코드 분석과 동적 테스트를 통해 CSV Injection(Formula Injection / Spreadsheet Injection) 취약점을 탐지하는 스킬. 사용자 입력이 CSV/Excel 내보내기에 반영될 때 수식 주입을 통해 데이터 탈취나 명령어 실행이 가능한지 분석하고 검증한다. 사용자가 'CSV injection 찾아줘', 'CSV 인젝션 스캔', 'formula injection', '수식 삽입', 'spreadsheet injection', 'Excel 수식 주입', 'CSV 점검' 등을 요청할 때 이 스킬을 사용한다."
grep_patterns:
  - "json2csv"
  - "csv-stringify"
  - "csv-writer"
