...
```mermaid
flowchart TD
    User["사용자: /noah-sast:scan"] --> S0["Step 0: grep 인덱싱"]
...
├── skills/
│   └── scan/
│       └── SKILL.md                      # 통합 오케스트레이터 (/noah-sast:scan)
│
├── hooks/
│   └── hooks.json                        # 보안 후크 (파괴적 endpoint 차단)
│
...
│   └── validate_actuator.py              # Actuator 파괴적 endpoint 차단
│
├── sub-skills/                           # 내부 서브스킬 (오케스트레이터에서 호출)
│   ├── scan-report/                      # 보고서 작성
│   │   ├── SKILL.md
