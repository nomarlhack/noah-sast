...
> 38개 개별 스캐너를 오케스트레이션하여 정적 분석 → 연계 분석 → 동적 테스트 → 보고서 생성까지 수행합니다.
...
개별 스캐너가 코드베이스를 중복 탐색하는 것을 방지하기 위해, 38개 스캐너의 grep 패턴을 일괄 실행하여 인덱스를 생성합니다.
...
1. 38개 스캐너의 `phase1.md` frontmatter에서 `grep_patterns:` 추출
...
> grep 패턴은 각 스캐너의 `phase1.md` 최상단 YAML frontmatter (`grep_patterns:`)에 정의되어 있다. Step 0 grep 인덱싱 에이전트가 38개 phase1.md frontmatter를 직접 파싱하여 사용한다. 별도 통합 yml 파일은 없다.
