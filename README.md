...
- [아키텍처](#아키텍처)
- [실행 프로세스](#실행-프로세스)
  - [Step 0: 패턴 사전 인덱싱](#step-0-패턴-사전-인덱싱)
  - [Step 1: 프로젝트 스택 파악](#step-1-프로젝트-스택-파악)
  - [Step 2: 스캐너 선별](#step-2-스캐너-선별)
  - [Step 3: 분석 실행](#step-3-분석-실행)
  - [Step 4: 보고서 생성](#step-4-보고서-생성)
- [스캐너 목록](#스캐너-목록)
- [개별 스캐너 구조](#개별-스캐너-구조)
- [분석 방법론](#분석-방법론)
- [보고서 파이프라인](#보고서-파이프라인)
...
All-Scanner는 Claude Code의 **스킬(Skill)** 시스템 위에 구축된 통합 취약점 분석 프레임워크입니다.
...
Kotlin, Java, TypeScript, JavaScript, Python, Go, Ruby, PHP, C#, Rust 등 80+ 확장자를 지원하며, Spring Boot, React, Vue, Django, Express 등 주요 프레임워크의 보안 패턴을 인식합니다.
...
│  │ 인덱싱    │  │ 스택 파악  │  │ 스캐너선별 │  │  분석 실행  │  │
│  └──────────┘  └──────────┘  └──────────┘  └────────────┘  │
│       │                            │              │         │
│       ▼                            ▼              ▼         │
│  ┌──────────┐              ┌────────────┐  ┌────────────┐  │
│  │  grep     │              │  scanner-   │  │  Phase 1   │  │
│  │  인덱싱   │              │  selector   │  │  그룹 에이전트│  │
│  │  에이전트  │              │  .py        │  │  (12개 병렬) │  │
│  └──────────┘              └────────────┘  └─────┬──────┘  │
│       │                                          │         │
│       ▼                                          ▼         │
│  ┌──────────────────┐                    ┌────────────┐    │
│  │ /tmp/scan_index/  │                    │  연계 분석   │    │
│  │  xss-scanner.json │                    │  에이전트    │    │
│  │  ssrf-scanner.json│                    └─────┬──────┘    │
│  │  ...              │                          │          │
│  └──────────────────┘                          ▼          │
│                                          ┌────────────┐    │
...
│  ┌─────────────────────────────────────────────▼────────┐  │
│  │                    Step 4: 보고서 생성                  │  │
│  │  scan-report → scan-report-review → HTML 변환 → 검증  │  │
│  └─────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
```
...
```
34개 스캐너의 SKILL.md frontmatter
         │
         ▼  grep_patterns 추출
    ┌─────────────────┐
    │ grep -rn 일괄실행 │  → 80+ 확장자 화이트리스트
    └────────┬────────┘    → node_modules 등 제외
             │
             ▼  스캐너별 JSON 저장
    /tmp/scan_index_{project}_{timestamp}/
    ├── xss-scanner.json          # {"innerHTML": ["app/Comment.jsx:18"], ...}
    ├── ssrf-scanner.json
    ├── sqli-scanner.json
    └── ... (34개)
```
...
3. 패턴 인덱스 JSON 읽기
4. **Sink-first** (패턴 인덱스 기반) + **Source-first** (입력값 추적) 병행 분석
...
- 후보 간의 의존 관계 분석 (A의 결과가 B의 전제조건이 되는지)
- 완전 체인 / 부분 체인 분류
- 체인별 Step-by-step 공격 시나리오 도출
...
`scan-report` 스킬이 통합 보고서를 생성합니다.

```
스켈레톤 작성 → 서브에이전트 병렬 상세 작성 → assemble_report.py 조립
    → scan-report-review 검증 → md_to_html.py 변환 → 링크/정량 검증
```
...
```
Sink-first (패턴 인덱스 기반)          Source-first (입력값 추적)
─────────────────────────           ─────────────────────────
패턴 인덱스에서 Sink 위치 확인         @RequestParam, req.query 등
         │                                    입력 지점에서 출발
         ▼                                         │
Sink 코드를 Read로 확인                              ▼
         │                              입력값이 위험 함수에
...
| 파일 | 설명 |
|------|------|
| `all-scan-report.md` | 마크다운 원본 |
| `all-scan-report.html` | 브라우저용 HTML (단일 파일, 외부 의존성 없음) |
...
사용자: "전체 취약점 스캔해줘"
    │
    ▼
[Step 0] grep 인덱싱 에이전트 실행 (자동)
    │
    ▼
[Step 1] 프로젝트 스택 분석 (자동)
    │
    ▼
[Step 2] 스캐너 선별 결과 테이블 출력 (자동)
    │
    ▼
[Step 3] Phase 1 정적 분석 (12개 그룹 병렬, 자동)
    │
    ├── 후보 0건 → Step 4로 직행
    │
    └── 후보 1건 이상 → 연계 분석 → 동적 테스트 정보 요청
         │
         ▼
    사용자: 테스트 환경 정보 제공 또는 "동적 테스트 안 해도 돼"
         │
         ▼
    [Step 3] Phase 2 동적 분석 (순차, 정보 제공 시)
         │
         ▼
    [Step 4] 보고서 생성 → 브라우저에서 열기
```
...
│   └── SKILL.md                     # 소스코드 대조 검증 프로세스
│
├── chain-analysis/                  # 연계 분석
│   └── SKILL.md                     # 취약점 간 연계 분석 프로세스
│
