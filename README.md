...
```mermaid
flowchart LR
    subgraph Phase1["Phase 1: 정적 분석 (병렬)"]
        G1["url-navigation\nxss, dom-xss, open-redirect"]
        G2["response-header\ncrlf, host-header, http-method"]
        G3["db-query\nsqli, nosqli"]
        G4["process-execution\ncmd-injection, ssti"]
        G5["server-request\nssrf, pdf-generation"]
        G6["file-system\npath-traversal, file-upload, zipslip"]
        G7["xml-serialization\nxxe, xslt, deserialization"]
        G8["auth-protocol\njwt, oauth, saml, csrf, idor"]
        G9["client-rendering\nredos, css-injection, prototype-pollution"]
        G10["infra-config\nhttp-smuggling, sourcemap, subdomain"]
        G11["data-export\ncsv-injection"]
        G12["protocol-check\ngraphql, websocket, soap, ldap"]
    end

    Phase1 -->|후보 목록| Master["후보 마스터 목록"]
    Master -->|2건 이상| ChainA["Phase 2: 연계 분석"]
    ChainA --> Dynamic["Phase 3: 동적 분석\n(순차)"]
    Dynamic -->|최종 판정| Result["확인됨 / 후보 / 안전"]

    style Phase1 fill:#16213e,stroke:#0f3460,color:#eee
    style Master fill:#e94560,stroke:#e94560,color:#fff
    style ChainA fill:#533483,stroke:#533483,color:#fff
    style Dynamic fill:#0f3460,stroke:#0f3460,color:#fff
...
flowchart LR
    Skeleton["스켈레톤 작성\n(메인 에이전트)"]
    Sub["서브에이전트\n상세 섹션 병렬 작성"]
    Assemble["assemble_report.py\n조립 + 요약 테이블 생성"]
    ReviewStep["scan-report-review\n소스코드 대조 검증"]
    Convert["md_to_html.py\nHTML 변환"]
    Validate["validate_links.py\nvalidate_report.py"]
    Output["noah-sast-report.md\nnoah-sast-report.html"]

    Skeleton --> Assemble
    Sub --> Assemble
    Assemble --> ReviewStep
    ReviewStep -->|오류 수정| ReviewStep
    ReviewStep --> Convert --> Validate
    Validate -->|PASS| Output
    Validate -->|FAIL: 파일 삭제| Assemble
