...
작업 디렉토리에 독립된 프로젝트가 여러 개 있으면 자동 감지하여 **프로젝트 단위로 분리 스캔 → 결과 통합 → 단일 보고서** 생성.

| 구조 | 판별 | 처리 |
|------|------|------|
| **단일 프로젝트** | 루트에 매니페스트 있음 | 전체를 한 번에 스캔 |
| **모노레포** | 루트에 상위 매니페스트(workspaces, settings.gradle 등) 있음 | 단일 프로젝트로 취급 |
| **멀티 프로젝트** | 루트에 매니페스트 없고, 하위 디렉토리 2개+ 각각 독립 매니페스트 보유 | Step 0~3-1을 프로젝트별 반복, Step 3-2~4는 통합 실행 |

멀티 프로젝트 시 후보 ID에 프로젝트 약칭을 하이픈 접두사로 부여합니다 (예: `hubrix-SSRF-1`, `reacto-JWT-2`).
...
| `phase1-group-agent.md` | Phase 1 그룹 에이전트 프롬프트. 그룹 내 스캐너를 순차 실행하며 각 스캐너의 phase1.md + 패턴 인덱스를 읽고 분석. 결과를 `===SCANNER_BOUNDARY===` 구분자로 반환하는 형식 정의 |
