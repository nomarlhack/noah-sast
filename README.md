...
Phase 1 정적 분석 완료 후, 구조화된 스캐너(grep 패턴 기반)가 놓칠 수 있는 취약점을 AI가 코드를 직접 읽으며 자율적으로 탐색합니다.

**기본 프롬프트 (프롬프트당 1개 에이전트, 병렬 실행):**
...
각 에이전트(`prompts/ai-discovery-agent.md`)의 분석 흐름:

1. 프로젝트 컨텍스트 + Phase 1 마스터 목록 요약 수신
2. 라우트/컨트롤러, 미들웨어, 데이터 접근 계층 등 핵심 코드 직접 탐색
...
| `ai-discovery-agent.md` | AI 자율 취약점 탐색 에이전트 프롬프트. grep 패턴에 구속되지 않고 코드를 직접 읽으며 취약점을 탐색. Phase 1 중복 제거, `===AI_DISCOVERY===` 형식으로 후보 반환 |
