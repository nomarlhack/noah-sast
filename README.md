...
| 스킬 | 명령 | 설명 |
|------|------|------|
| **SAST** | `/noah-8719:sast` | 41개 취약점 스캐너 + AI 자율 탐색으로 정적 분석 + 동적 테스트 + 보고서 생성 |
...
Phase 1 정적 분석 완료 후, 구조화된 스캐너(grep 패턴 기반)가 놓칠 수 있는 취약점을 AI가 코드를 직접 읽으며 자율적으로 탐색합니다. **단일 에이전트**가 3개 프롬프트를 순차 실행하며, 이전 프롬프트에서 읽은 코드와 인사이트가 컨텍스트에 누적되어 점점 더 깊은 탐색이 가능합니다.
...
에이전트(`prompts/ai-discovery-agent.md`)의 분석 흐름:

1. 프로젝트 컨텍스트 + Phase 1 마스터 목록 요약 수신
...
| `ai-discovery-agent.md` | AI 자율 취약점 탐색 에이전트 프롬프트. 단일 에이전트가 3개 프롬프트를 순차 실행하며 컨텍스트 누적으로 점진적 심화 탐색. Phase 1 중복 제거, `===AI_DISCOVERY===` 형식으로 후보 반환 |
