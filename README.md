...
| 스킬 | 명령 | 설명 |
|------|------|------|
| **SAST** | `/noah-8719:scan` | 41개 취약점 스캐너로 정적 분석 + 동적 테스트 + 보고서 생성 |
...
```mermaid
flowchart TD
    User["사용자: /noah-8719:scan"] --> S0["Step 0: grep 인덱싱"]
...
`--plugin-dir` 플래그로 플러그인 경로를 지정합니다. 이후 `/noah-8719:scan`으로 실행합니다.
