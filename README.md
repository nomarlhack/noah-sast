...
| 스킬 | 명령 | 설명 |
|------|------|------|
| **SAST** | `/noah-8719:sast` | 41개 취약점 스캐너로 정적 분석 + 동적 테스트 + 보고서 생성 |
...
`sub-skills/sast-report/SKILL.md`에 따라 통합 보고서를 생성합니다. 상세 흐름은 [보고서 파이프라인](#보고서-파이프라인) 참조.
...
```bash
python3 sub-skills/sast-report/assemble_report.py
```

| 항목 | 설명 |
|------|------|
| **입력** | 스크립트 내부 변수: `skeleton`(보고서 뼈대), `subagent_results`(스캐너별 상세 MD 목록), `chain_analysis`(연계 분석 데이터 dict/JSON), `report_name`(출력 파일명) |
...
```bash
python3 sub-skills/sast-report/md_to_html.py [보고서명]
```
...
| **기능** | (1) 총괄 대시보드 카드 (확인됨/후보/이상 없음/미적용 건수) — `N건`/`N개` 접미사로 파싱 (2) `### Scanner` 헤딩을 collapsible `<details>` 섹션으로 변환 (3) `#### N. 제목`에 `id="vuln-N"` 앵커 부여 (4) 요약 테이블의 `#vuln-N` 링크 생성 (5) 코드 블록 구문 하이라이팅 스타일 (6) `<details>` 내부 앵커로의 스크롤을 위한 JS 삽입 |
...
```bash
python3 sub-skills/sast-report/validate_links.py <html_file>
```
...
```bash
python3 sub-skills/sast-report/validate_report.py <기대_POC_건수> [보고서명] [--chain-analysis]
```

| 항목 | 설명 |
|------|------|
| **입력** | 기대 POC 건수 (취약점 + 체인 POC 합계), 보고서명 (기본: `noah-sast-report`), `--chain-analysis` 플래그 |
...
### sub-skills/sast-report-review/

#### `SKILL.md` + `checklist.md`

MD 보고서 작성 후 HTML 변환 전에 실행되는 정확성 검증 에이전트. `checklist.md`의 체크리스트에 따라 보고서의 각 취약점을 소스코드와 대조하여 부정확한 기술(잘못된 파일 경로, 코드 스니펫 불일치, Source→Sink 흐름 오류)을 식별하고 MD 파일을 직접 수정.
...
> 플러그인 설치 후 사용. 또는 `sast`, `소스코드 취약점 스캔`으로도 트리거됩니다.
...
`--plugin-dir` 플래그로 플러그인 경로를 지정합니다. 이후 `/noah-8719:sast`으로 실행합니다.
