...
Phase 1 완료 후, **모든 프로젝트의 후보를 하나의 후보 마스터 목록으로 통합**한다. 각 후보 ID에 프로젝트 접두사를 부여한다 (예: `hubrix:SSRF-1`, `reacto:JWT-1`). 이후 Step 3-2(연계 분석) ~ Step 4(보고서)는 통합된 마스터 목록으로 **한 번만** 실행하여 **단일 보고서**를 생성한다.
