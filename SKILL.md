...
2. 후보 ID 충돌을 방지하기 위해 프로젝트별 번호 오프셋을 적용한다 (1번째 프로젝트: 1~999, 2번째: 1001~1999, 3번째: 2001~2999).
3. 통합된 `master-list.json`을 작업 디렉토리에 저장한다. 후보의 `file` 필드에 프로젝트 디렉토리가 포함되므로 (`hubrix-main/src/...`) 프로젝트 구분은 이 필드로 식별한다.
