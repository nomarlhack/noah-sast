...
```bash
python3 <NOAH_SAST_DIR>/tools/cache_manager.py status <PROJECT_ROOT>
```
...
- **풀 스캔 후**: `python3 <NOAH_SAST_DIR>/tools/cache_manager.py save <PROJECT_ROOT> <PATTERN_INDEX_DIR>`
- **증분 후**: `python3 <NOAH_SAST_DIR>/tools/cache_manager.py merge <PROJECT_ROOT> <PATTERN_INDEX_DIR>`
...
**Phase 1 결과 수집 및 추적:**
- 그룹 에이전트가 반환한 결과를 `===SCANNER_BOUNDARY===` 구분자와 `[스캐너명]` 태그로 분리하여, 각 스캐너의 후보를 개별적으로 추출한다. 파싱이 어려운 경우 `<NOAH_SAST_DIR>/tools/parse_phase1_output.py`에 에이전트 출력을 전달하여 구조화된 JSON으로 변환할 수 있다 (레거시 `=== 스캐너명 ===` 형식도 하위 호환 지원).
...
Phase 1에서 후보가 2건 이상 발견된 경우, `<NOAH_SAST_DIR>/tools/chain-analysis/SKILL.md`에 정의된 프로세스에 따라 **연계 분석 에이전트**를 실행한다. 후보가 1건 이하이면 이 단계를 건너뛴다.
...
사용자가 필요한 정보를 제공하면, 후보가 발견된 모든 스캐너에 대해 동적 분석을 수행한다. **[필수] 스캐너당 1 에이전트 원칙을 따른다. 여러 스캐너의 동적 분석을 하나의 에이전트에 묶지 않는다.** 각 에이전트 프롬프트에는 `<NOAH_SAST_DIR>/scanners/<scanner-name>/phase2.md`와 `<NOAH_SAST_DIR>/guidelines/phase2.md` 경로를 포함한다.
...
Step 3에서 수집한 모든 개별 스캐너의 결과를 **`scan-report` 스킬(`<NOAH_SAST_DIR>/tools/scan-report/SKILL.md`)에 전달**하여 통합 보고서를 생성한다.
...
**[필수] scan-report의 Step 3(MD 조립) 완료 후, Step 4(HTML 변환) 이전에 `scan-report-review` 스킬을 실행하여 보고서 정확성을 검증한다.** `<NOAH_SAST_DIR>/tools/scan-report-review/SKILL.md`를 읽고, 보고서 MD 파일 경로와 프로젝트 루트 경로를 전달한다. 리뷰에서 부정확한 내용이 발견되면 MD 파일이 수정된 후 HTML 변환으로 진행한다.

## 유의사항 (메인 에이전트 — 일반)

- **[필수] 모든 동적 테스트는 `guidelines/phase2.md` 지침 11(도메인 분류)을 통과한 sandbox 도메인에서만 수행한다.** 사용자가 제공한 도메인이 prod/cbt로 분류되면 동적 테스트를 절대 수행하지 않고 sandbox URL을 요청한다. 분류 불명/staging이면 명시적 확인 후에만 진행.
...
- Phase 1 에이전트: `<NOAH_SAST_DIR>/guidelines/phase1.md`
- Phase 2 에이전트: `<NOAH_SAST_DIR>/guidelines/phase2.md`
