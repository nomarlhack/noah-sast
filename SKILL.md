...
Phase 1 완료 후, **모든 프로젝트의 후보를 하나의 후보 마스터 목록으로 통합**한다. 각 후보 ID는 기존 형식을 유지하되 충돌을 방지하기 위해 프로젝트별 번호 범위를 분리한다 (예: hubrix는 `SSRF-1`~, reacto는 `SSRF-101`~). 후보의 `**위치**:` 필드에 프로젝트 경로가 포함되므로 프로젝트 구분은 이 필드로 식별한다. 이후 Step 3-2(연계 분석) ~ Step 4(보고서)는 통합된 마스터 목록으로 **한 번만** 실행하여 **단일 보고서**를 생성한다.
