...
사용자가 필요한 정보를 제공하면, 후보가 발견된 모든 스캐너에 대해 동적 분석을 수행한다. **[필수] 스캐너당 1 에이전트 원칙을 따른다. 여러 스캐너의 동적 분석을 하나의 에이전트에 묶지 않는다.** 각 에이전트 프롬프트에는 `<NOAH_SAST_DIR>/scanners/<scanner-name>/phase2.md`와 `<NOAH_SAST_DIR>/prompts/guidelines-phase2.md` 경로를 포함한다.
