...
**[필수] 메인 에이전트는 아래 프롬프트를 축약·요약·재작성하지 않는다.** 특히 단계 3의 JSON 저장 형식 예시(dict 형식: `{"패턴": ["파일:라인", ...]}`)는 한 글자도 빠뜨리지 않고 그대로 포함해야 한다.
...
**[필수] Step 2-1 출력 형식 (모든 35개 스캐너 포함):**
...
- Phase 1 공통 지침: `<NOAH_SAST_DIR>/agent-guidelines-phase1.md`
...
- phase1.md: <NOAH_SAST_DIR>/scanners/open-redirect-scanner/phase1.md
