...
#### Step 0-0: 캐시 확인 (증분 분석)

grep 인덱싱 에이전트를 생성하기 전에, 이전 실행의 캐시가 유효한지 확인한다.

```bash
python3 <NOAH_SAST_DIR>/utils/cache_manager.py status <PROJECT_ROOT>
```
...
| 출력 | 의미 | 조치 |
|------|------|------|
| `CACHE_HIT: 변경 없음` | 코드 변경 없음 | `INDEX_DIR:` 뒤의 경로를 `PATTERN_INDEX_DIR`로 사용. **Step 0-1~0-2를 건너뛰고 Step 1로 진행.** |
| `CACHE_STALE: N개 파일 변경` | 일부 변경 | `INDEX_DIR:` 뒤의 경로를 `PATTERN_INDEX_DIR`로 사용. Step 0-1에서 **변경 파일만** 증분 grep 실행. |
| `CACHE_MISS` | 캐시 없음/무효 | 기존과 동일하게 풀 스캔 실행. |

> **캐시 디렉토리**: `<PROJECT_ROOT>/.noah-sast-cache/`. `.gitignore`에 추가할 것을 권장한다.

#### Step 0-1: grep 인덱싱 에이전트 생성

**[필수] grep 인덱싱 에이전트 생성 전에 메인 에이전트가 직접 Bash로 고유 경로를 생성한다.**

**풀 스캔 (CACHE_MISS)**: 아래 Bash 명령을 실행하여 완성된 디렉토리 경로 문자열을 얻는다:
...
**캐시 저장 (풀 스캔 또는 증분 완료 후):**

grep 인덱싱이 완료되면, 결과를 프로젝트 로컬 캐시에 저장하여 다음 실행에서 증분 분석이 가능하도록 한다.

- **풀 스캔 후**: `python3 <NOAH_SAST_DIR>/utils/cache_manager.py save <PROJECT_ROOT> <PATTERN_INDEX_DIR>`
- **증분 후**: `python3 <NOAH_SAST_DIR>/utils/cache_manager.py merge <PROJECT_ROOT> <PATTERN_INDEX_DIR>`

캐시 저장이 실패해도 분석 진행에는 영향 없다 (다음 실행에서 풀 스캔으로 폴백).
...
```bash
python3 <NOAH_SAST_DIR>/scanner-selector.py <PATTERN_INDEX_DIR> <PROJECT_ROOT>
```
...
**[필수] 메인 에이전트는 그룹 에이전트 프롬프트 본문(절차/결과 형식)을 인라인으로 복사하지 않는다.** 대신 아래 형식의 짧은 프롬프트를 사용한다. `<NOAH_SAST_DIR>`/`<PATTERN_INDEX_DIR>`는 resolve된 실제 경로 문자열로 치환한다.

```
<NOAH_SAST_DIR>/prompts/phase1-group-agent.md를 Read 도구로 읽고 그 안의 절차를 정확히 따르세요.
변수: NOAH_SAST_DIR=<NOAH_SAST_DIR>
...
`prompts/phase1-group-agent.md`에 결과 반환 형식(`=== 스캐너명 ===` 구분자)과 공통 절차가 정의되어 있으므로, 메인 에이전트는 본문을 절대 축약·재작성·재인용하지 않는다.

> **주의: Phase 1 완료 시점에서 보고서 파일(.md/.html)을 생성하지 않는다.** 보고서는 Step 4에서만 작성한다.

**Phase 1 결과 수집 및 추적:**
- 그룹 에이전트가 반환한 결과를 `===SCANNER_BOUNDARY===` 구분자와 `[스캐너명]` 태그로 분리하여, 각 스캐너의 후보를 개별적으로 추출한다. 파싱이 어려운 경우 `<NOAH_SAST_DIR>/utils/parse_phase1_output.py`에 에이전트 출력을 전달하여 구조화된 JSON으로 변환할 수 있다 (레거시 `=== 스캐너명 ===` 형식도 하위 호환 지원).
...
Phase 1에서 후보가 2건 이상 발견된 경우, `<NOAH_SAST_DIR>/utils/chain-analysis/SKILL.md`에 정의된 프로세스에 따라 **연계 분석 에이전트**를 실행한다. 후보가 1건 이하이면 이 단계를 건너뛴다.
...
사용자가 필요한 정보를 제공하면, 후보가 발견된 모든 스캐너에 대해 동적 분석을 수행한다. **[필수] 스캐너당 1 에이전트 원칙을 따른다. 여러 스캐너의 동적 분석을 하나의 에이전트에 묶지 않는다.** 각 에이전트 프롬프트에는 `<NOAH_SAST_DIR>/scanners/<scanner-name>/phase2.md`와 `<NOAH_SAST_DIR>/agent-guidelines-phase2.md` 경로를 포함한다.
...
Step 3에서 수집한 모든 개별 스캐너의 결과를 **`scan-report` 스킬(`<NOAH_SAST_DIR>/utils/scan-report/SKILL.md`)에 전달**하여 통합 보고서를 생성한다.
...
**[필수] scan-report의 Step 3(MD 조립) 완료 후, Step 4(HTML 변환) 이전에 `scan-report-review` 스킬을 실행하여 보고서 정확성을 검증한다.** `<NOAH_SAST_DIR>/utils/scan-report-review/SKILL.md`를 읽고, 보고서 MD 파일 경로와 프로젝트 루트 경로를 전달한다. 리뷰에서 부정확한 내용이 발견되면 MD 파일이 수정된 후 HTML 변환으로 진행한다.

## 유의사항 (메인 에이전트 — 일반)

- **[필수] 모든 동적 테스트는 `agent-guidelines-phase2.md` 지침 11(도메인 분류)을 통과한 sandbox 도메인에서만 수행한다.** 사용자가 제공한 도메인이 prod/cbt로 분류되면 동적 테스트를 절대 수행하지 않고 sandbox URL을 요청한다. 분류 불명/staging이면 명시적 확인 후에만 진행.
...
- Phase 1 에이전트: `<NOAH_SAST_DIR>/agent-guidelines-phase1.md`
- Phase 2 에이전트: `<NOAH_SAST_DIR>/agent-guidelines-phase2.md`
